 ...
 */
void generate_tweet(MarkovChain *markov_chain, MarkovNode *first_node, int max_length) {
  if (first_node == NULL) {
  ...
  markov_chain->print_func(first_node->data); // Print the first word of the tweet
  printf(" ");
  ...
  while (is_period == 0 && words_in_tweet < max_length - 1) {
    ...
    markov_chain->print_func(temp->data); // Print the word
    printf(" ");
  ...
  markov_chain->print_func(temp->data); // Print the last word of the tweet
